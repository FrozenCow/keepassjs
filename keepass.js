...
exports.userPassword = function(password) {
	return crypt.SHA256(UTF8ToBytes(password), {asBytes: true});
}
...
exports.readDatabaseFromBytes = function(userKeys, bytes, result, error) {
	...
	var finalKey = (function fabricateDecryptionKey(userKeys) {
		function transformKey(raw, keyseed, rounds) {
			...
			var opt = {mode: new crypt.mode.ECB(crypt.pad.NoPadding), iv: iv, asBytes: true};
			for (var i = 0; i < rounds; i++) {
				key = crypt.AES.encrypt(key, keyseed, opt);
			}
			return crypt.SHA256(key, {asBytes: true});
		}
		
		var concatKeys = [];
		userKeys.forEach(function(userKey) { concatKeys = concatKeys.concat(userKey); });
		var compositeKey = crypt.SHA256(concatKeys, {asBytes: true});
		var transformedMasterKey = transformKey(compositeKey, header.transformSeed, header.transformRounds);
		return crypt.SHA256(header.masterSeed.concat(transformedMasterKey), {asBytes: true});
	})(userKeys);
	...
	var decrypter = ({
		'31c1f2e6bf714350be5805216afc5aff': function AES_CBC_PKCS7(key, iv) {
			var options = {mode: new crypt.mode.CBC(crypt.pad.pkcs7), iv: iv, asBytes: true};
			return function(input) {
				return crypt.AES.decrypt(input, key, options);
			};
		}
	...
	content = (function CheckAndConcatHashedBlocks(content) {
		...
		while(true) {
			...
			var calculatedHash = crypt.SHA256(blockBytes, {asBytes: true});
			for (var i=0;i<32;i++) {
	...
	var randomStream = ({
		...
		2: salsa20
	}[header.randomStreamID])(crypt.SHA256(header.protectedStreamKey, {asBytes: true}));
